...
if(args[1] == "aç") {
    ...
    .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
    .setThumbnail(client.user.avatarURL())
    .setDescription("BlackCode **bakım** modu aktif edildi!")
    .addField("Yetkili", message.author)
    .addField("Sebep", reason)
    .setColor("BLUE")
    client.channels.cache.get(ch.webstatus).send(`BlackCode Sitesi Bakıma Geçti! Sebebi: **${reason}**`).then(a => { 
...
if(args[1] == "kapat") {
    ...
    const tebriklerkanks = new Discord.MessageEmbed()
    .setAuthor("#AlwaysBlack", client.user.avatarURL())
    .setThumbnail(client.user.avatarURL())
    .setDescription(`Tebrikler, **${d.reason}** sebebiyle kapanmış olan BlackCode'u tekrar açtınız.`)
    .setColor("GREEN")
    .setTitle("BAŞARILI")
    message.channel.send(tebriklerkanks);
    client.channels.cache.get(ch.webstatus).messages.fetch(d.bakimmsg).then(a => { a.edit(`~~ BlackCode **${d.reason}** sebebi ile __bakım__ moduna alındı. ~~`, bakimbitti) } )
    const bakimbitti = new Discord.MessageEmbed()
    .setAuthor("#AlwaysBlack", client.user.avatarURL())
    .setThumbnail(client.user.avatarURL())
    .setColor("GREEN")
    .setDescription(`BlackCode Tekrar Aktif! \n[Tıkla ve sitemize git!](https://web.blackcode.cf)`)
    .setFooter("BlackCode");
