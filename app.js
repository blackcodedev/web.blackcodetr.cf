...
const renderTemplate = (res, req, template, data = {}) => {
        ...
        if(req.isAuthenticated()) {
            if(client.guilds.cache.get(settingsc.serverID).members.cache.get(req.user.id)) {
                ...
                next();
                    ...
                    res.redirect('/error?code=200&message=Sitemiz geçici olarak bakımda.') 
                }
            } else {
                res.redirect('/error?code=200&message=Sitemiz geçici olarak bakımda.') 
            }
  ...
  app.get("/dc", (req, res) => {
    res.redirect('https://bit.ly/blackcodetr');
  });
  app.get("http", (req, res) => {
    res.redirect('https://web.blackcodetr.cf');
  });
  ...
  app.get("/ukqzn", (req, res) => {
    res.redirect('https://bit.ly/ukqzndc');
  }); 
app.get("/error", (req, res) => {
  ...
  app.get("/code/:code", checkMaintence, checkAuth, async (req,res) => {
    ...
    if(!client.guilds.cache.get(settingsc.serverID).members.cache.get(req.user.id)) return res.redirect("/error?code=403&message=Burayı görebilmek için discord sunucumuza girmelisin.");
    ...
    if(!client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.javascript)) return res.redirect("/error?code=403&message=Bu kodlara erişimin yok.");
    }
    if(docs.codeCategory == "html") {
    if(!client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.html)) return res.redirect("/error?code=403&message=Bu kodlara erişimin yok.");
    }
    if(docs.codeCategory == "subs") {
    if(!client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.altyapilar)) return res.redirect("/error?code=403&message=Bu kodlara erişimin yok.");
    }
    if(docs.codeCategory == "5invites") {
    if(!client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.besdavet)) return res.redirect("/error?code=403&message=Bu kodlara erişimin yok.");
    }
    if(docs.codeCategory == "10invites") {
    if(!client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.ondavet)) return res.redirect("/error?code=403&message=Bu kodlara erişimin yok.");
    }
    if(docs.codeCategory == "15invites") {
    if(!client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.onbesdavet)) return res.redirect("/error?code=403&message=Bu kodlara erişimin yok.");
    }
    if(docs.codeCategory == "20invites") {
    if(!client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.yirmidavet)) return res.redirect("/error?code=403&message=Bu kodlara erişimin yok.");
    }
    if(docs.codeCategory == "bdfd") {
    if(!client.guilds.cache.get(config.serverID).members.cache.get(req.user.id).roles.cache.get(roles.bdfd)) return res.redirect("/error?code=403&message=Bu kodlara erişimin yok.");
    }
  ...
  app.post("/code-request", checkMaintence, checkAuth, async (req,res) => {
     client.users.fetch(req.user.id).then(a => {
       ...
       client.channels.cache.get(channels.request).send(new Discord.MessageEmbed()
                            ...
                            .setDescription(`User **[${a.username}#${a.discriminator}](https://blackcodetr.cf/user/${a.id})** requested the code named **${rBody['kodadi']}**.`)
  ...
  app.get("/bot/:botID/vote", checkMaintence, checkAuth, async (req,res) => {
    ...
    if(!botdata) return res.redirect("/error?code=404&message=Geçersiz bir bot ID'si girdiniz.");
    if(!req.user.id === botdata.ownerID || req.user.id.includes(botdata.coowners)) {
      if(botdata.status != "Approved") return res.redirect("/error?code=404&message=Geçersiz bir bot ID'si girdiniz.");
    }
  ...
  app.post("/bot/:botID/vote", checkMaintence, checkAuth, async (req,res) => {
    ...
    if(!a.bot) return res.redirect("/error?code=404&message=Geçersiz bir bot ID'si girdiniz.");
    if(!a) return res.redirect("/error?code=404&message=Geçersiz bir bot ID'si girdiniz.");
    if(rBody['coowners']) {
  ...
  app.get("/bot/:botID", checkMaintence, async (req,res,next) => {
    ...
    if(botdata.status != "Approved") {
      if(req.user.id == botdata.ownerID || botdata.coowners.includes(req.user.id)) {
        ...
        res.redirect("/error?code=404&message=Hey! bu botu sadece botun sahipleri düzenleyebilir.");
      }
    } else {
      let coowner = new Array()
      botdata.coowners.map(a => {
...
await profiledata.findOneAndUpdate({userID: req.user.id}, {$set: {instagram: rBody['instagram']}}, {upsert:true})
  ...
  app.get('*', function(req, res){
    res.status(404).redirect('/error?code=404&message=Galiba yolunu kaybettin.')
  });
